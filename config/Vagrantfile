...
hostname        = "ATELIER-WEB-SERV"

#
# vhost configuration
# other vhosts can be added by using the vhost facility inside the vm
#$ sudo vhost -s vhost_hostname -d vhost_docroot
vhost_s_public         = "play.findyourway.local"
vhost_d_public         = "/var/www/FindYourWay/webclient"

vhost_s_api_public       = "backend.findyourway.local"
vhost_d_api_public        = "/var/www/FindYourWay/api/public"

vhost_s_api_private         = "admin.findYourWay.local"
vhost_d_api_private      = "/var/www/FindYourWay/api/private"
...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "api_private", type:"shell", inline: "sudo vhost -s #{vhost_s_api_private} -d #{vhost_d_api_private}"
  config.vm.provision "api_public", type:"shell", inline: "sudo vhost -s #{vhost_s_api_public} -d #{vhost_d_api_public}"
